...
DEFINE_string(table_name, "zhipeng", "Table name");
...
static void* Set(void* arg) {
  noah::test_client::Cluster* cluster = static_cast<noah::test_client::Cluster*>(arg);
  int index = g_index++;
...
static void* List(void* arg) {
  ...
  int32_t size = 0;
  std::unique_ptr<noah::test_client::Iterator> iterator(cluster->NewIterator(FLAGS_table_name));
  while (!brpc::IsAskedToQuit()) {
    begin_time_us = butil::gettimeofday_us();
...
bool get_key_values() {
  ...
  if (FLAGS_data_file.empty()) {
    ...
    noah::test_client::FileRead file_read(FLAGS_data_file.c_str());
...
int main(int argc, char* argv[]) {
  ...
  noah::test_client::Options options(FLAGS_meta_group, FLAGS_meta_conf);
  options.auto_partition_num = FLAGS_partition_num;
  options.auto_duplicate_num = FLAGS_duplicate_num;
  noah::test_client::Cluster cluster(options);
  if (!cluster.Init()) {
